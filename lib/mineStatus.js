const mineStatus = {
  ...
  initializeStatus : function(){
    this.status = Array.from(Array(this.row), () => new Array(this.col).fill(0));
    this.visited = Array.from(Array(this.row), () => new Array(this.col).fill(false));
    this.map = Array.from(Array(this.row), () => new Array(this.col).fill(0));
    this.plantMine(this.map, this.numofMine);
  },
