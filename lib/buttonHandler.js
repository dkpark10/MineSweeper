...
module.exports = {
  ...
  setFlagMark: function (minedata, coord) {
    ...
    if(visited[y][x] === true){
      ...
      flagBoard[y][x] = !this.Flag;
      minedata.setFlagBoard(flagBoard);
      return { coord: coord, status: this.buttonStatus.SETFLAG, number: aroundNumberOfBoard[y][x] };
    }
  },
  ...
  isRightStickFlag: function(mineData, coord){
    ...
    const mineBoard = mineData.getMineBoard();
    const visited = mineData.getVisited();
    const numberOfBoard = mineData.getAroundNumberOfBoard();
    const aroundNumberOfCurCoord = numberOfBoard[y][x];
    ...
    for (let i = y - 1; i <= y + 1; i++) {
      for (let j = x - 1; j <= x + 1; j++) {
        ...
        if ( mineBoard[i][j] !== flagBoard[i][j] && visited[i][j] === false)  
          wrongcnt++;
        if (mineBoard[i][j] === flagBoard[i][j] && visited[i][j] === false) {
          returnData.returndata.push({coord:[i,j], status:this.buttonStatus.NUMBERCELL, number:numberOfBoard[i][j]});
          rightcnt++;
        }
      }
    }
    ...
    if(rightcnt === aroundNumberOfCurCoord)
      ...
      return { coord: coord, status: this.buttonStatus.END, number: 0 };
    else
      return { coord: coord, status: this.buttonStatus.NOTHING, number: 0 };
  },
