module.exports = {
  ...
  isRange: function (y,x, mineBoard) {
    const row = mineBoard.row;
    const col = mineBoard.col;
    if (y < 0 || x < 0 || y >= row || x >= col) return false;
    else return true;
  },

  getAroundNumOfCell:function (y, x, mineBoard) {
    let number = 0;
    let board = mineBoard.mineBoard;
    for (let i = y - 1; i <= y + 1; i++) {
      for (let j = x - 1; j <= x + 1; j++) {
        if (this.isRange(i, j, mineBoard) === false) continue;
        if (board[i][j] === this.isMine) {
          number++;
        }
      }
    }return number;
  ...
  isBombClick: function (statusOfCell) {
    return statusOfCell === 0 && 
  }
}
