...
module.exports = {
  ...
  mineData: function (mineData) {
    ...
    const extraCell = mineData.extraCell;
  ...
  setFlagMark: function (minedata, coord) {
    ...
    if (currentStatus === this.status().getInitStatus()) {
      ...
      minedata.setFlagBoard(flagBoard);
      return { coord: coord, status: 'RELIVEFLAG', number: 0 };
    }
  },
    ...
    for (let i = y - 1; i <= y + 1; i++) {
      for (let j = x - 1; j <= x + 1; j++) {
        ...
        if (mineBoard[i][j] === flagBoard[i][j] && visited[i][j] === false) {
          returnData.responsedata.push({coord:[i,j], status:'DISABLED', number:mineData.getAroundNumberOfBoard()[i][j]});
          rightcnt++;
        }
      }
    }
