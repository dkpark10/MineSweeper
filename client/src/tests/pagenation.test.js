...
describe('페이지네이션 테스트', () => {
  ...
  test('페이지 범위 앞', async () => {
    for (let currentPage = 1; currentPage <= 1 + half; currentPage += 1) {
  ...
  test('페이지 범위 중간', async () => {
    for (let currentPage = 2 + half; currentPage < lastPage - half; currentPage += 1) {
      pageInfo.currentPage = currentPage;
      const [beginPage] = calculBeginPage(pageInfo);
  ...
  test('페이지 범위 마지막', async () => {
    for (let currentPage = lastPage; currentPage >= lastPage - half; currentPage -= 1) {
      pageInfo.currentPage = currentPage;
      const [beginPage] = calculBeginPage(pageInfo);
